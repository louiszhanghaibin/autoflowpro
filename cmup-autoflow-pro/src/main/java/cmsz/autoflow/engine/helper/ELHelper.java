...
//必须创建对象
  ...
  class StaticFunctionHandler extends handleChain {
    ...
    Object handle(Object obj, String elParameter) {
      ...
      Object[] objargs = null;
      if (StringHelper.isNotEmpty(paras)) {
   ...
   * 重新組裝字符串, 并解析 #｛静态函数调用｝
   * 
   * @param el 需要解析的字符串 例如 "settleDate=#{cmsz.autoflow.engine.helper.DateHelper.getDate(-1)}" ，当前日期是2015/04/10,则返回
   *        settleDate="20150409"  (包括=之前的都是)
   * @return String
   */
